...
Two of these earn their place by being the failure modes this protocol already
worries about, stated by someone else in more detail than a role card can afford.
...
| Not delegated | Why |
|---|---|
| `superpowers:writing-plans` | `plan.md`'s YAML blocks carry `file_scope` and `depends_on`, which the orchestrator mechanically enforces. Prose cannot be enforced. It also writes plans inside the repo, which this protocol forbids. |
